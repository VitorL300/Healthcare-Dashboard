...
# 1. Carregamento e cópia da base de dados
@st.cache_data
def load_data():
    df = pd.read_excel('data/Healthcare.xlsx')
    return df
...
with st.sidebar.expander('Advanced Filters'):
    insurance_provider = st.multiselect('Insurance Provider',
    ...
    age_group = st.multiselect('Age Group',
                                options= ['Kids', 'Adults', 'Mid', 'Senior', 'Old'],
                                default= ['Kids', 'Adults', 'Mid', 'Senior', 'Old'])
    
    
# 3. Aplicação dos filtros criados   
df = base_df[
...
prev_end = end_date - timedelta(days= total_days)
...
# *Billing Amount = Valor de Cobrança / len() retorna o número total de itens
...
# *Patient Intelligence, Financial Intelligence, Raw Data - Inteligência do Paciente, Inteligência Financeira, Dados Brutos
